...
class Segment:
    def __init__(self,
        ...
        self.inertia = biorbd.Matrix3d(float(inertia[0, 0]), float(inertia[0, 1]), float(inertia[0, 2]),
...
def huygens(inertia, mass, com, new_com):
    ...
    b = d[1]
    c = d[2]
    return inertia + mass * np.array([[b ** 2 + c ** 2, -a * b, -a * c],
                                      [-a * b, a ** 2 + c ** 2, -b * c],
                                      [-a * c, -b * c, a ** 2 + b ** 2]])
...
def merge_segment(Segment_master, Segment_child):
    ...
    new_com = np.average(np.concatenate((Segment_master.com.to_array()[:, np.newaxis],
                                         child_com_in_master[:, np.newaxis]), axis=1),
                         weights=[Segment_master.mass, Segment_child.mass], axis=1)

    # compute the new inertia matrix at the new com
    new_inertia = huygens(Segment_master.inertia.to_array(), Segment_master.mass, Segment_master.com.to_array(),
                          new_com) \
    ...
    merge_seg = Segment(name=Segment_master.name,
...
class Model:
    ...
    def add_segment(self,
        ...
        self.model.AddSegment(segment.name,
                              segment.parent,
    ...
    def add_marker(self, marker: Marker):
        ...
        self.model.addMarker(marker.position, marker.name, marker.parent,
                             marker.technical, marker.anatomical, marker.axis_to_remove, parent_int)
        print('hello')


my_model = Model('humanoid_10_dof')
torso = Segment(name="Torso",
                parent="ROOT",
                rtinmatrix=False,
                rt=np.zeros(3),
                xyz=np.zeros(3),
                ...
                inertia=np.array([[5, 0, 0],
                                  [0, 0, 0],
                                  [0, 0, 0]]),
                mesh=np.array([[0, 0, 0],
                               [0, 0, 0.7]])
                )
head = Segment(name="Head",
               parent="Torso",
               rtinmatrix=False,
               rt=np.zeros(3),
               ...
               inertia=np.array([[0.12, 0, 0],
                                 [0, 0, 0],
                                 [0, 0, 0]]),
               mesh=np.array([[0, 0, 0],
                              [0, 0, 0.24]]))
r_thigh = Segment(name="RThigh",
                  parent="Torso",
                  rtinmatrix=False,
                  rt=np.zeros(3),
                  xyz=np.zeros(3),
                  ...
                  inertia=np.array([[0.7, 0, 0],
                                    [0, 0, 0],
                                    [0, 0, 0]]),
                  mesh=np.array([[0, 0, 0],
                                 [0, 0, -0.41]]))
l_thigh = Segment(name="LThigh",
                  parent="Torso",
                  rtinmatrix=False,
                  rt=np.zeros(3),
                  xyz=np.zeros(3),
...
r_forearm = Segment(name="RForearm",
                    parent="RArm",
                    rtinmatrix=False,
                    rt=np.zeros(3),
...
l_forearm = Segment(name="LForearm",
                    parent="LArm",
                    rtinmatrix=False,
                    rt=np.zeros(3),
                    xyz=np.array([0, 0, -0.3]),
...
r_foot_contact = Contact(name="RFoot",
                         parent="RShank",
                         position=np.array([0, 0, -0.46]),
                         axis="yz",
                         )
r_foot_marker = Marker(name="RFoot",
                       parent="RShank",
                       position=np.array([0, 0, -0.46]),
                       )
l_foot_marker = Marker(name="LFoot",
                       parent="LShank",
                       position=np.array([0, 0, -0.46]),
                       )

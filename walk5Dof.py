...
def prepare_ocp(
    ...
    objective_functions.add(ObjectiveFcn.Mayer.MINIMIZE_COM_VELOCITY, index=2, node=Node.ALL, weight=0.01)
    ...
    phase_transitions.add(anti_symmetric_cyclic_transition, first_index=3, second_index=4, phase_pre_idx=0, weight=10)
    phase_transitions.add(anti_symmetric_cyclic_transition, first_index=5, second_index=6, phase_pre_idx=0, weight=10)

    phase_transitions.add(
        anti_symmetric_cyclic_transition, first_index=3 + n_q, second_index=4 + n_q, phase_pre_idx=0, weight=0.01
    )
    phase_transitions.add(
        anti_symmetric_cyclic_transition, first_index=5 + n_q, second_index=6 + n_q, phase_pre_idx=0, weight=0.01
    )
...
def main():
    model_path = "models/Humanoid5Dof.bioMod"
    n_shooting = 10
    ...
    print(sol.states["qdot"][3, 0] - sol.states["qdot"][4, -1])
    print(sol.states["qdot"][4, 0] - sol.states["qdot"][3, -1])
